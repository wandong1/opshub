    ...
    <div class="content">
      <div class="content-header">
        <div class="header-info">
          ...
          <a-tag v-if="activeTab === 'cluster'" color="red" size="small">
        ...
        <div class="header-actions">
          <a-button type="primary" @click="handleCreateRole" :loading="loading">
            <icon-plus />
            添加角色
          </a-button>
      ...
      <!-- 集群角色列表 -->
      <div v-if="activeTab === 'cluster'" class="role-list" :loading="loading">
        ...
        <a-empty v-else description="暂无集群角色，点击左下角按钮创建" :image-size="100" />
      </div>

      <!-- 命名空间角色列表（显示所有命名空间的角色定义） -->
      <div v-else class="role-list" :loading="loading">
        ...
        <a-empty v-else description="暂无命名空间角色，点击左下角按钮创建" :image-size="100" />
      </div>
    </div>
    ...
    >
      <div class="create-role-content">
        ...
        <div class="form-section">
          <div class="section-title">
            <icon-link />
            <span>API 组</span>
            <a-tag size="small" color="gray">步骤 1</a-tag>
          </div>
          <div class="form-item">
        ...
        <div class="form-section" :class="{ disabled: createForm.apiGroups.length === 0 }">
          <div class="section-title">
            <icon-folder />
            <span>资源</span>
            <a-tag size="small" :type="createForm.apiGroups.length > 0 ? 'info' : 'info'">步骤 2</a-tag>
          </div>
          <div class="form-item">
            <label>选择资源</label>
            <a-select
        ...
        <div class="form-section" :class="{ disabled: createForm.resources.length === 0 }">
          <div class="section-title">
            <icon-settings />
            <span>操作权限</span>
            <a-tag size="small" :type="createForm.resources.length > 0 ? 'info' : 'info'">步骤 3</a-tag>
          </div>
          <div class="form-item">
            <label>选择操作</label>
            <a-select
              ...
              style="width: 100%"
            >
              <a-option label="get - 查看单个资源" value="get" />
              <a-option label="list - 列出资源" value="list" />
              <a-option label="watch - 监听资源" value="watch" />
              <a-option label="create - 创建资源" value="create" />
              <a-option label="update - 更新资源" value="update" />
              <a-option label="patch - 部分更新" value="patch" />
              <a-option label="delete - 删除资源" value="delete" />
              <a-option label="deletecollection - 批量删除" value="deletecollection" />
              <a-option label="* - 所有权限" value="*" />
            </a-select>
          </div>
        </div>
        ...
        <div v-if="createForm.resources.length > 0 && createForm.verbs.length > 0" class="rule-preview">
          ...
          <div class="preview-content">
            <div class="preview-item">
              ...
              <a-tag v-for="api in createForm.apiGroups" :key="api" size="small" type="primary">
                {{ api || 'core' }}
              </a-tag>
            </div>
            <div class="preview-item">
              <span class="preview-label">资源:</span>
              <a-tag v-for="res in createForm.resources" :key="res" size="small" color="green">
                {{ res }}
              </a-tag>
            </div>
            <div class="preview-item">
              <span class="preview-label">操作:</span>
              <a-tag v-for="verb in createForm.verbs" :key="verb" size="small" :type="getVerbType(verb)">
                {{ verb }}
              </a-tag>
            </div>
          </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <a-button @click="createDialogVisible = false" size="large">取消</a-button>
          <a-button
            type="primary"
...
const handleCreateRoles = async () => {
  ...
  try {
    ...
    Message.success(`已为 ${nsList.length} 个命名空间创建角色`)
...
.menu-item {
  ...
  &.active {
    background: linear-gradient(90deg, rgba(22, 93, 255, 0.1) 0%, rgba(22, 93, 255, 0.03) 100%);
    color: #165dff;
...
.create-role-dialog {
  ...
  .rule-preview {
    background: linear-gradient(135deg, #f5f7fa 0%, rgba(22, 93, 255, 0.06) 100%);
    border-radius: 8px;
    padding: 16px;
