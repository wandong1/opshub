<template>
  <div class="configmap-list">
    ...
    <!-- YAML 弹窗 -->
    <el-dialog v-model="yamlDialogVisible" :title="`ConfigMap YAML - ${selectedConfigMap?.name}`" width="900px" class="yaml-dialog">
...
import { Search, Key, Document, Delete } from '@element-plus/icons-vue'
...
const loadConfigMaps = async () => {
  ...
  try {
    ...
    configMapList.value = []
    ElMessage.error('获取 ConfigMap 列表失败')
  } finally {
    loading.value = false
  }
}
...
// 编辑 ConfigMap
const handleEdit = (row: ConfigMapInfo) => {
  emit('edit', row)
}
